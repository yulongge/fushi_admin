<template>
  <div class="app-container">
    <div class="filter-container">
      <el-form ref="form" class="search-form" :model="listQuery" label-width="100px" :inline="true">
    ...
    >
      <el-table-column
        ...
        width="55"
      />
      <el-table-column :label="$t('product.bianhao')" width="200px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.orgName }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('product.caijishijian')" align="center">
        <template slot-scope="{row}">
          <span>{{ row.caijishijian }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('product.baohudianya')" width="200px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.baohudianya }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('product.fushisulv')" width="200px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.fushisulv }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.actions')" align="center" fixed="right" width="300" class-name="small-padding fixed-width">
...
import { getDztzList } from '@/api/dztz'
...
export default {
  ...
  methods: {
    ...
    analysic() {
      this.$router.push(`/charts/mix-chart`)
    }
  }
}
</script>
