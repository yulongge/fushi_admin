<template>
  <div class="user-container">
    ...
    >
      ...
      <el-table-column :label="$t('table.avatar')" width="140px" align="center">
        <template slot-scope="{row}">
          <img v-if="row.avatar" :src="row.avatar" class="avatar-table">
      ...
      <el-table-column :label="$t('table.birthday')" width="180px" align="center">
        <template slot-scope="{row}">
          <span v-if="row.birthday">{{ row.birthday | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
          <span v-else>--</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.phone')" width="160px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.phone || '--' }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.orgCode')" width="110px" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column :label="$t('table.actions')" align="center" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}">
          ...
          <el-link type="primary" class="option-item" @click="handlePwd">{{ $t('table.pwd') }}</el-link>
          <el-link type="primary" class="option-item" @click="handleDelete(row, $index)">{{ $t('table.delete') }}</el-link>
          <el-link type="primary" class="option-item" @click="handleForze(row, $index)">{{ $t('table.forze') }}</el-link>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="560px">
...
export default {
  ...
  methods: {
    ...
    handleForze(row, index) {
      this.$confirm('确定要冻结此用户, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
